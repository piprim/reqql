...
func handleProducts(sqlTmpl *template.Template, queryFunc reqql.QueryFuncArgs) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := products.Proceed(r.Context(), sqlTmpl, &filter, &result, queryFunc); err != nil {
...
func main() {
	...
	if err != nil {
		log.Fatalf("parse sql template: %v", err)
	}
	...
	mux.HandleFunc("POST /products", handleProducts(sqlTmpl, queryFunc))

	log.Println("listening on " + port)
	log.Println("open your browerser at http://localhost" + port)
