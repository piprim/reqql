...
type QueryFuncArgs = func(ctx context.Context, dest any, query string, args ...any) e.PrivateErrorI
type QueryFuncArg = func(ctx context.Context, dest any, query string, arg any) e.PrivateErrorI

type QueryFunc interface{ QueryFuncArgs | QueryFuncArg }

type NoInputType = any
type SelectFuncType[T any] func(*T) (string, e.PrivateErrorI)
type QueryFuncType[T any] func(*T) (string, any, e.PrivateErrorI)
type FromFuncType[T any] func(*T) (string, e.PrivateErrorI)
type WhereFuncType[T any] func(*T) (string, e.PrivateErrorI)
type OrderFuncType[T any] func(*T) (string, e.PrivateErrorI)
type LimitFuncType[T any] func(*T) (string, e.PrivateErrorI)
type OffsetFuncType[T any] func(*T) (int, e.PrivateErrorI)
...
func DefaultQueryParser[T any](_ *T) (sql string, arg any, err e.PrivateErrorI) {
	return fmt.Sprintf(queryFormat, selectAll, fromDefault), nil, nil
}

func WhereTrueFunc[T any](_ *T) (string, e.PrivateErrorI) {
	return whereTrue, nil
}

func NoOrderFunc[T any](_ *T) (string, e.PrivateErrorI) {
	return noOrder, nil
}

func NoLimitFunc[T any](_ *T) (string, e.PrivateErrorI) {
	return noLimit, nil
}

func NoOffsetFunc[T any](_ *T) (int, e.PrivateErrorI) {
	return noOffset, nil
}
...
// See [NewProcessor].
func (p *Processor[InputType, QueryFuncType]) Proceed(ctx context.Context, input *InputType, dest any) e.PrivateErrorI {
...
// SELECT SelectFunc(…) FROM FromFunc() WHERE WhereFunc(…) ORDER BY OrderFunc(…) LIMIT LimitFunc(…) OFFSET OrderFunc(…)
func (q *Queryer[InputType]) WithOderFunc(f OrderFuncType[InputType]) *Queryer[InputType] {
	q.orderFunc = f
...
func (q *Queryer[InputType]) Parse(input *InputType) (sql string, arg any, err e.PrivateErrorI) {
...
func Proceed[InputType any, QF QueryFunc](
	...
	input *InputType, dest any, queryFunc QF) e.PrivateErrorI {
	...
	if arg != nil && q.arg != nil {
		...
		return e.Internal(errors.New("Parse query function return not nil"), data)
	}
	...
	case QueryFuncArgs:
		...
		default:
			...
			return e.Internal(errors.New(fmt.Sprintf("function need []any, %T given", args)), data)
		}
	}

	return e.Internal(errors.New("unpredictible error (wtf)"), nil)
}
